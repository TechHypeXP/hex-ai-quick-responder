...
suite('Extension Test Suite', () => {
    ...
    setup(() => {
        ...
        showQuickPickStub = sinon_1.default.stub(vscode.window, 'showQuickPick');
        showInformationMessageStub = sinon_1.default.stub(vscode.window, 'showInformationMessage');
        // Stub configuration
    ...
    test('handles predefined dialog responses', async () => {
        ...
        assert.ok(showInformationMessageStub.calledWith('saveChanges', { modal: true }, 'Yes'));
    });
    test('handles AI-processed responses with rate limiting', async () => {
        ...
        const processMessageStub = sinon_1.default.stub(providerManager, 'processMessage')
            .callsFake(async () => {
            callCount++;
            if (callCount === 1) {
        ...
        await vscode.commands.executeCommand('hex-quick-responder.respond');
        assert.ok(showInformationMessageStub.calledWith('AI processing failed: Error: rate limit exceeded'));
        // Advance time past backoff period
        clock.tick(30001);
        // Second attempt should succeed
        await vscode.commands.executeCommand('hex-quick-responder.respond');
        assert.ok(showInformationMessageStub.calledWith('unknownQuestion', { modal: true }, 'AI generated response'));
        assert.strictEqual(callCount, 2);
        processMessageStub.restore();
    });
    test('adds new response mapping', async () => {
        const inputStub = sinon_1.default.stub(vscode.window, 'showInputBox');
        ...
        assert.ok(showInformationMessageStub.calledWith('Hex Quick Responder: Added mapping "newQuestion" → "newResponse"'));
    });
    ...
    test('handles window state changes with rate limiting', async () => {
        const windowStateEventEmitter = new vscode_1.EventEmitter();
        const disposable = new vscode_1.Disposable(() => { });
        sinon_1.default.stub(vscode.window, 'onDidChangeWindowState')
    ...
    test('handles configuration changes', async () => {
        ...
        await vscode.commands.executeCommand('hex-quick-responder.respond');
        assert.ok(showInformationMessageStub.calledWith('newDialog', { modal: true }, 'Response'));
    });
